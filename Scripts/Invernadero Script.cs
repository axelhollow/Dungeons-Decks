...
public class InvernaderoScript : MonoBehaviour
{
    ...
    private List<string> prefabsNames = new List<string> { "Manzana"};
    private List<float> tiemposDeCrafting = new List<float> { 5f};
    private List<List<int>> combinacionesCorrectas = new List<List<int>>
    {
        new List<int> { 5 },//1 manzana = 2 manzanas
    ...
    private IEnumerator CraftDespuesDeTiempo(float tiempo, int prefabIndex)
    {
        ...
        if (prefabIndex != -1 && prefabIndex < prefabsNames.Count)
        {
            ...
            if (prefab != null)
            {
                ...
                Instantiate(prefab, posicionOriginal + new Vector3(0f, 2.1f, -8f), rotacionOriginal);
